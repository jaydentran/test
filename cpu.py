...
def get_processor_name():
    if plf.system() == "Windows":
        try:
            import winreg
            key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"Hardware\Description\System\CentralProcessor\0")  # NOQA
...
ONE_MILLION = 10**6
t_empty = timeit.Timer('for i in range({}):\n    pass\n'.format(ONE_MILLION))  # NOQA
t = timeit.Timer('s = 0\nfor i in range({}):\n    s = s + i\nprint(s)'.format(ONE_MILLION))  # NOQA
